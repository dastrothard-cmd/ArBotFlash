...
function renderTraitSnapshot(profile) {
  const traitItems = [
    ['Form', ['plantForm', 'plantWoodiness', 'plantLongevity', 'heightBand']],
    ['Bark', ['barkType', 'barkColour']],
    ['Leaves / foliage', ['leafArrangement', 'leafType', 'leafShape', 'leafMargin', 'foliageColour']],
    ['Flowers', ['flowerColour', 'floweringSeason', 'phenology']],
    ['Fruit / cones / seed', ['fruitType', 'fruitColour', 'seedType']],
    ['Habitat', ['habitat', 'substrate']],
    ['Region', ['region', 'bioregion', 'country']],
    ['Status', ['establishment', 'regionalStatus', 'endemicStatus']]
  ];
  const cards = traitItems.map(([label, keys]) => {
    const values = keys.map(key => normaliseTraitValue(profile?.[key])).filter(Boolean);
    if (!values.length) return '';
    return `<article class="trait-card"><strong>${escapeHtml(label)}</strong><span>${escapeHtml(values.join(' · '))}</span></article>`;
  }).filter(Boolean);
  byId('profileTraitSnapshot').innerHTML = cards.length
    ? cards.join('')
    : '<p class="muted">No field-trait cards are attached to this profile yet.</p>';
}
...
function renderDetailedProfile(profile) {
  ...
  if (profile.media?.length) {
    byId('profileMedia').innerHTML = profile.media.map(item => `<figure>
      <a href="${escapeHtml(item.source_page_url || item.original_url)}" target="_blank" rel="noopener noreferrer"><img src="${escapeHtml(item.display_url || item.storage_key || item.original_url)}" data-fallback-src="${escapeHtml(item.original_url || '')}" alt="${escapeHtml(item.caption || profile.commonName)}" loading="lazy"></a>
