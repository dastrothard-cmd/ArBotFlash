...
const state = {
  ...
  openFilterGroups: new Set(['Taxonomy', 'Plant identification', 'Universal']),
...
function renderFilters() {
  elements.filters.innerHTML = groups().map(group => {
    ...
    const open = state.openFilterGroups.has(group) ? 'open' : '';
    return `<details data-group="${escapeHtml(group)}" ${open}><summary>${escapeHtml(group)} <span>${defs.length}</span></summary>${defs.map(({ definition, options }) => `
  ...
  elements.filters.querySelectorAll('details[data-group]').forEach(details => {
    details.addEventListener('toggle', () => {
      if (details.open) state.openFilterGroups.add(details.dataset.group);
      else state.openFilterGroups.delete(details.dataset.group);
    });
  });
...
function normaliseTraitValue(value) {
  if (value === undefined || value === null || value === '') return '';
  if (Array.isArray(value)) return value.filter(Boolean).join(', ');
  return String(value);
}

function sectionText(profile, key) {
  const section = profile?.profileSections?.find(item => item.section_key === key);
  return section?.body_markdown || '';
}

function firstUsefulText(profile, traitKeys, sectionKeys = []) {
  const traitValues = traitKeys.map(key => normaliseTraitValue(profile?.[key])).filter(Boolean);
  if (traitValues.length) return traitValues.join(' · ');
  const sectionValues = sectionKeys.map(key => sectionText(profile, key)).filter(Boolean);
  return sectionValues[0] || '';
}
...
function renderTraitSnapshot(profile) {
  const cardSpecs = [
    ['Form', ['plantForm', 'plantWoodiness', 'plantLongevity', 'heightBand'], ['summary']],
    ['Bark / key look', ['barkType', 'barkColour'], ['identifying_features']],
    ['Leaves / foliage', ['leafArrangement', 'leafType', 'leafShape', 'leafMargin', 'foliageColour'], ['identifying_features']],
    ['Flowers', ['flowerColour', 'floweringSeason', 'phenology'], ['phenology']],
    ['Fruit / cones / seed', ['fruitType', 'fruitColour', 'seedType'], ['identifying_features']],
    ['Habitat', ['habitat', 'substrate'], ['habitat']],
    ['Region', ['region', 'bioregion', 'country'], ['distribution']],
    ['Status', ['establishment', 'regionalStatus', 'endemicStatus'], ['conservation_status', 'regional_status']]
  ];
  const cards = cardSpecs.map(([label, traitKeys, sectionKeys]) => {
    const value = trimCardText(firstUsefulText(profile, traitKeys, sectionKeys));
    if (!value) return '';
    return `<article class="trait-card"><strong>${escapeHtml(label)}</strong><span>${escapeHtml(value)}</span></article>`;
  }).filter(Boolean);
  byId('profileTraitSnapshot').innerHTML = cards.length
    ? cards.join('')
    : '<p class="muted">Field ID cards are pending for this taxon.</p>';
}
...
function renderDetailedProfile(profile) {
  ...
  if (profile.media?.length) {
    byId('profileMedia').innerHTML = profile.media.map(item => {
      const sourceImage = item.original_url || item.display_url || item.storage_key || '';
      const localImage = item.display_url || item.storage_key || item.original_url || '';
      return `<figure>
        <a href="${escapeHtml(item.source_page_url || item.original_url)}" target="_blank" rel="noopener noreferrer"><img src="${escapeHtml(sourceImage)}" data-fallback-src="${escapeHtml(localImage)}" alt="${escapeHtml(item.caption || profile.commonName)}" loading="lazy"></a>
    ...
    byId('profileMedia').querySelectorAll('img[data-fallback-src]').forEach(img => {
...
elements.save.addEventListener('click', () => {
  const saved = {
    ...
    sourcePack: 'ArbotFlash working catalogue',
...
async function refreshPackStatus() {
  try {
    ...
    if (installed) {
      ...
      elements.packMessage.textContent = 'Offline pack tools are hidden in this UI pass.';
...
async function useInstalledPack() {
  ...
  elements.status.className = 'connection-status offline';
  elements.databaseSummary.textContent = `${installed.taxa.length} cards loaded · ${installed.manifest.coverage.enrichedTaxa} sourced profiles · ${installed.manifest.coverage.localMediaCount} licensed images.`;
  return true;
}
...
async function useStaticFallback() {
  ...
  elements.status.className = 'connection-status offline';
  elements.databaseSummary.textContent = `${taxa.length} cards loaded from the static fallback. Some expanded profile data may require the API deployment.`;
}
...
async function initialise() {
  ...
  try {
    ...
    elements.status.textContent = 'Database connected';
    elements.status.className = 'connection-status connected';
    elements.databaseSummary.textContent = `${payload.meta.taxonCount} cards · ${payload.meta.enrichedTaxa} sourced profiles · ${payload.meta.locallyStoredMediaCount} licensed images · ${payload.meta.profileShellCount} empty shells`;
  } catch (error) {
    try {
...
async function installSelectedPack() {
  ...
  elements.packMessage.textContent = 'Installing offline data.';
  try {
